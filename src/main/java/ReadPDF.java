...
//            String filePath = "C:\\Users\\ceag9\\OneDrive\\Documents\\CEAG\\Personal\\Financial\\BankStatements\\2021\\Chase\\CEA-CC-4540\\20210507-statements-4540-.pdf";
    ...
    public static void getPreviousBalance(String line, int index) {
        String paymentRegex = "^(Previous\\s+Balance)(\\s+)\\$?(?<amount>(\\d+\\.\\d+)$)";
        Pattern r = Pattern.compile(paymentRegex);
        Matcher m = r.matcher(line);
        if (m.find( )) {
            beginBalance = beginBalance.add(new BigDecimal(m.group(3)));
            System.out.println("Line[" + index + "] : Previous Balance: $" + m.group(3) + "]");
        }
    }

    public static void getPayment(String line, int index) {
        String paymentRegex = "(?<mm>^\\d{2})\\/(?<dd>\\d{2})(\\s+)(?<description>(.)+)(\\s+)(\\-)(?<amount>(\\d+\\.\\d+)$)";
        Pattern r = Pattern.compile(paymentRegex);
        Matcher m = r.matcher(line);
        if (m.find( )) {
            totalPayment = totalPayment.add(new BigDecimal(m.group(8)));
    ...
    public static void getPurchase(String line, int index) {
        ...
        Matcher m = r.matcher(line);
        if (m.find( )) {
            totalPurchase = totalPurchase.add(new BigDecimal(m.group(7)));
